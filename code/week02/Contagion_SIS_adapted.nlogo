...
to infect-susceptibles ;; S -> I
  ask turtles with [not recovered? and not infected?] [
...
"pen-1" 1.0 0 -7500403 true "" "plot (count turtles with [recovered?]) / num-turtles"
...
This model is original material created by Paul E. Smaldino. The model has been modified during class by Eric Araújo.

## Questions and notes

Changing speed and transmissibility effects and calculating the proportion of infected per time step. What changes?
...
<experiments>
  ...
  <experiment name="sis vs sir" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count (turtles with [infected?]) / num-turtles</metric>
    <metric>count (turtles with [recovered?]) / num-turtles</metric>
    <enumeratedValueSet variable="num-turtles">
      <value value="300"/>
    </enumeratedValueSet>
    ...
    <steppedValueSet variable="transmissibility" first="0.01" step="0.01" last="0.5"/>
    <steppedValueSet variable="recovery-rate" first="0.01" step="0.01" last="0.2"/>
    <enumeratedValueSet variable="init-infected">
      <value value="3"/>
    </enumeratedValueSet>
