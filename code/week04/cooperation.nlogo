...
to go
  ;; stop condition
  if (all? people [strategy = 0] or all? people [strategy = 1]) [stop]
  play-game
  evolve
  recolor
  tick
...
to play-game
  ask people [
    ...
    let neighbors-C count (people-on neighbors4) with [strategy = 1]
    let neighbors-D count (people-on neighbors4) with [strategy = 0]
    ;; check my strategy
    if (strategy = 1)[
...
to evolve
  ask people [
    let best-neighbor max-one-of (people-on neighbors4) [payoff]
