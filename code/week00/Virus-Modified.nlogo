turtles-own [
  ...
  immune?              ;; if true, turtle is part of the removed (R) group
  sick-time            ;; how long, in weeks, the turtle has been infectious
]

globals [
  %infected            ;; what % of the population is infectious
  %immune              ;; what % of the population is immune (R)
  ]
...
to recover? ;; turtle procedure
  if sick-time > duration-sickness [
...
"never infected" 1.0 0 -10899396 true "" "plot count turtles with [ not sick? and not immune? ]"
...
This model simulates a SIR (Susceptible - Infected - Recovered) model for the transmission of a virus. This model is based on a previous model from the Netlogo library called Virus.

It is main purpose is to be used during the CS300b - Agent Modeling course at Calvin University for the Fall semester of 2024, and its changes were implemented by prof. Eric Araujo.

## HOW IT WORKS

The model is initialized with a maximum of 300 people, of which 30 are infected.  People move randomly about the world in one of three states: healthy but susceptible to infection (green), sick and infectious (red), and healthy and immune/removed (gray). 
...
We are also interested on investigating the movement of people. So our implementation changes how many patches are moved forward by the agents each time step.

### Degree of immunity

If a person has been infected and recovered, how immune are they to the virus?  We often assume that immunity lasts a lifetime and is assured, but in some cases immunity wears off in time and immunity might not be absolutely secure.  In this model, immunity is secure, and lasts for the rest of the simulation.
...
How long is a person infected before they either recover?  This length of time is essentially the virus's window of opportunity for transmission to new hosts. In this model, duration of infectiousness is determined by the DURATION-SICKNESS slider.
...
The DURATION-SICKNESS slider determines the minimum number of days before an infected person is able to recover.

The CHANCE-RECOVER slider controls the likelihood that an infection will end in recovery.  When this slider is set at zero, for instance, the infection is permanent.

The SETUP button resets the graphics and plots and randomly distributes NUMBER-PEOPLE in the view. All but the number of the slider INITIAL-INFECTED will define how many people are set to red, representing infected people.  The GO button starts the simulation and the plotting function.
...
The factors controlled by the three sliders interact to influence how likely the virus is to thrive in this population.  

Often there will initially be an explosion of infection since no one in the population is immune.  This approximates the initial "outbreak" of a viral infection in a population, one that often has devastating consequences for the humans concerned. 

By changing the range of movement of the nodes, you might also notice the virus spreading less. That would explain how quarantine would help controlling the spread of diseases.
...
This model is an adaptation of the following model:
